...
public class SOminibossScripts : MonoBehaviour
{
    ...
    [SerializeField] GameObject PlateProjectile;
    ...
    private void Update()
    {
        ...
        if (isCheckingPlayer == true && (((int)transform.position.x + 3612) / 24 == ((int)playerPosition.position.x + 3612) / 24
            ...
            //&& Vector3.Distance(transform.position, playerPosition.position) > 20) //Here is chack for attacking
        {
            Attacks();
            if (animations.GetBool("Stay") == true)
                animations.Play("Armature|Walk");
